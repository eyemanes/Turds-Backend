...
export function validateFirebaseUid(uid) {
  ...
  // Support both Firebase UIDs and Privy DIDs
  // Firebase UIDs: 20-128 alphanumeric characters
  // Privy DIDs: did:privy: followed by alphanumeric characters
  if (uid.startsWith('did:privy:')) {
    // Privy DID format: did:privy: followed by alphanumeric
    if (!/^did:privy:[a-zA-Z0-9]{20,50}$/.test(uid)) {
      return { valid: false, error: 'Invalid Privy user ID format' };
    }
  } else {
